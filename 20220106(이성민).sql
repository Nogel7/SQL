...
SELECT buytbl.user_id,buytbl.order_number,buytbl.prod_name, 
       buytbl.group_name,buytbl.price,buytbl.amount,
       usertbl.user_name, usertbl.phone_number, usertbl.addr 
...
-- 테이블명을 풀로 적지 않고 별명으로 대체해서 호출할 수 있어 좀 더 편히합니다.
-- 현재 *로 처리된 부분을
-- buytbl의 구매 물품정보 전체에, 구매자 정보는 택배를 받기 위해서
-- 이름,주소,휴대폰번호만 있으면 되는 상황입니다.
-- 위에 적힌 컬럼조회하는 부분으로 고쳐주세요.(단, buytbl.*에 해당하는 구문은 쓰지 마세요)
SELECT b.user_id,b.order_number,b.prod_name,b.group_name,b.price,b.amount,u.user_name,u.phone_number,u.addr 
       FROM buytbl b INNER JOIN usertbl u 
       ON b.user_id = u.user_id;
       
-- WHERE절은 먼저 JOIN구문의 결과가 나온 상태에서, 추가 필터링만을 담당합니다.
-- 구매물품의 가격이 50000원 이상만 남기는 구문
SELECT b.user_id,b.order_number,b.prod_name,b.group_name,b.price,b.amount,
       u.user_name,u.phone_number,u.addr 
       FROM buytbl b INNER JOIN usertbl u 
       ON b.user_id = u.user_id 
       WHERE price >= 50000;
       
-- 위쪽 JOIN구문을 활용하시되, 가격이 200000만원 이하인 품목만 남겨주시고
...
-- INNERT JOIN을 넣은 자리에 대신 LEFT OUTER JOIN으로 고쳐주시기만 하면 작동합니다.
-- 테이블의 컬럼은 전체 출력합니다
SELECT * FROM usertbl LEFT OUTER JOIN buytbl ON usertbl.user_id = buytbl.user_id;

-- RIGHT JOIN인데, usetbl를 RIGHT에, buytbl을 LEFT에 두고 작성해주세요.
-- INNER JOIN을 넣은 저라에 대신 RIGHT OUTER JOIN으로 고쳐주시기만 하면 작동합니다.
-- 테이블 컬럼은 전체 출력합니다.
...
SELECT * FROM buytbl b LEFT OUTER JOIN usertbl u ON b.user_id = u.user_id
    UNION -- UNION구문은 위 아래 결과화면을 합쳐줍니다.(LEFT,RIGHT 사이에 넣으면 FULL)
...
-- 아래 구문을 full outer join으로 바꿔 주시고 결과를 보내주세요.
...
-- 한 번만 출력되게 만들어보세요.
SELECT s.user_name,user_addr,user_point FROM student s LEFT OUTER JOIN membership m ON s.user_name = m.user_name
    UNION
SELECT m.user_name,user_addr,user_point  FROM student s RIGHT OUTER JOIN membership m ON s.user_name = m.user_name;