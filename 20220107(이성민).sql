...
제한되는 점이 많지만, 이를 활용하는 경우가 비번하기 때문에
...
-- myQuery 실행하기
-- 저장된 구문의 ? 에 값을 전달하기 위해 USING전달값을 씁니다.
...
-- Oracle SQL에서는 sysdate,MYSQl 에서는 now()를 이용해
-- 현재 시각을 확인할 수 있습니다.
SELECT now();
...
SELECT '가나다' + 100; -- 숫자로 변환할 수 없는 문자 + 문자 => 정수끼리만 연산
...
SELECT CHAR_LENGTH('askdnqjwnoqdnosindoqin');

-- CONCAT(문자열1,문자열2.........);
SELECT CONCAT('가','나','다','라');

-- CONCAT_WS(구분자1, 문자열1, 문자열2.......);
-- 문자열과 문자열 사이를 구분자를 이용해 붙여줍니다.
SELECT CONCAT_WS('--','1','2','3','4','5','10','22');

-- FORMAT (숫자, 소수점자리)
SELECT FORMAT(1234.588475586295,3);
...
-- 소문자를 대문자로(UPPERCASE)
-- 대문자를 소문자로(LOWERCASE)
SELECT LCASE('abcdEFGH'), UCASE('abcdEFHG');

-- UPPER(문자열),LOWER(문자열)로 줄여서 쓰기도 합니다.
SELECT UPPER('abcdEFGH'), LOWER('abcdEFHG');
...
-- TRIM은 기본적으로 LTRIM + RTRIM 형식으로 양쪽의 모든 공백을 다 삭제해줍니다.
-- 공백이 아닌 특정 문자를 삭제하도록 구문을 지정할 수도 있습니다.
SELECT TRIM('        무야호         ');

-- 방향은 BOTH(양쪽다), LEADING(왼쪽), TRAILING(오른쪽)
-- 중 하나를 고르면 됩니다.
SELECT TRIM(TRAILING 'ㅋ' FROM 'ㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㄹㅇㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋ');
...
-- 프로시저 삭제는 DROP PROCEDURE 프로시저명; (프로시저명 오른쪽에 ()는 뻅니다);
