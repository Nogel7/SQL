...
-- 아래 코드는 지역 종류 7개(데이터 개수는 10개)를 파악하기 어렵습니다.
...
-- 데이터가 170000개에 가깝다보니 모든 데이터를 보여주지 안혹
-- 1000개만 보여주도록 설정되어있음
...
-- 기존 쿼리문에서의 조건절은 WHERE 절을 이용해서 처리했었습니다.
-- 하지만 GROUP BY 를 통해 집계하는 경우 WHERE절을 받을수가 없습니다.
-- 대신 조건절을 HAVING으로 대체해 사용합니다.
-- WHERE과 사용법은 완전히 동일합니다.
-- 우선 사용차별 총 구매액(buytbl)을 다시 구해보겠습니다.
-- (price * amount)를 활용해서 작성해주세요.
...
INSERT INTO testtbll VALUES(1,'홍길동',25);

-- id와 이름쪽에만 데이터를 넣을 때에는 컬럼명을 생략할 수 없습니다.
INSERT INTO testtbll(id, user_name) VALUES(2,'홍길동');

-- 만약 컬럼명 지정을 안 했지만 age에는 null을 넣고 싶다면
INSERT INTO testtbll VALUES (3, '이호영', null);

-- 만약 입력하는 컬럼(열)의 순서를 바꿔서 지정하는 경우는(id,user_name,age가 아닌 순으로 넣음)
-- 모든 컬럼에 값을 입력하는 상황이어도 컬럼명을 따로 기입해야 합니다.
INSERT INTO testtbll(user_name, age, id) VALUES ('채종훈',10,4);

SELECT * FROM testtbll;
