-- ict_practice 를 쓰도록 만들어주세요.
use ict_pracitce;

SELECT * FROM testtbl1;

-- 지금 testtbl1 같은 경우 id를 수동으로 입력해주고 있습니다.
-- 이 경우 매번 id값을 사용자가 계산해서 insert 해야하기때문에
...
-- CMD(커맨드라인)애서 testtbl2 컬럼에 데이터를 3개 더 입력해주세요.
-- 커맨드라인에서 조회까지 마쳐주세요.
SELECT * FROM testtbl2;
...
-- 증가문을 1이 아닌 3으로 세팅하는법
SET @@auto_increment_increment = 3;
-- 동물 3마리를 testtbl3에 추가해보세요.
INSERT INTO testtbl3 VALUES(null, '팬더', 5);
INSERT INTO testtbl3 VALUES(null, '펭귄', 13);
INSERT INTO testtbl3 VALUES(null, '거북이', 20);
SELECT * FROM testtbl3;
    
-- testtbl2도 3씩 증가하는지 다시 INSERT INTO명령어로 넣고 확인해보세요.
INSERT INTO testtbl2 VALUES(null, '고양이', 4);
INSERT INTO testtbl2 VALUES(null, '강아지', 4);
SELECT * FROM testtbl2;
-- 다시 증가량을 1로 돌려주세요.
...
-- 하나의 나이대만 우선 고쳐주세요.
UPDATE testtbl2 SET user_name = '팬더' WHERE age = 4;
SELECT * FROM testtbl2;
...
-- 1.36sec
-- CMD에서 TRUNCATE 를 이용해 삭제 후 소요시간을 적어주세요.
-- 0.06sec
...
-- 따라서 이 때 중복되는 데이터1건은 중복된다면 무시하고
...
-- ON DUPLICATE KET UPDATE 구문을 쓰면 됩니다.
...
-- 권한부여를 하는 방법관 권한 개념에 대해 알아보겠습니다.

-- workbench에서 계정생성 및 권한부여하기.
-- 1. 좌측의 navigator에서 Administration 탭을 클릭합니다.
-- 2. User and Privileges를 클릭합니다.
-- 3. 열리는 화면에서 좌하단 Add Account 를 클릭한 다음
...
-- 계정 접속 방법
-- 커맨더라인 접속시 mysql -u 계정명 -p 로 연결 가능
-- 접속 후 show databases; 를 이용해 DB 목록 확인가능
-- use mysql;
...
-- workbench에서 주기
-- Navigator 하단 Administration에서 유저 선택 후 Administrative Roles에서 주고싶은 권한을
-- 체크해주면 됩니다(DBA 선택시 모든 권한 획득)

-- CMD에서 전체가 아닌 일부 권한 주기
-- icttest 계정을 하나 새로 만들어주세요. 권한 주지마세요.
-- GRANT 권한1, 권한2... ON DB명.테이블명 TO 아이디@'%';
-- ict_practice 데이터베이스의 testtbl3 테이블에 대한 SELECT, DELETE 권한만
-- icttest 계정에 권한을 부여한다면
-- GRANT SELECT, DELETE ON ict_parctice.testtbl3 TO icttest@'%';
